...
contract NFTLotteryPool is ERC721Upgradeable, OwnableUpgradeable, ERC721HolderUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function buyTickets(uint256 numTickets) public payable {
    ...
    require(balanceOf(_msgSender()).add(numTickets) <= maxTicketsPerAddress, "Holding too many");
    require(tokenCounter.add(numTickets) <= maxTickets, "Exceeds max supply");
  ...
  function getRefund(uint256[] calldata ids) external nonReentrant {
    ...
    require(tokenCounter < minTicketsToSell || isRefundOpen, "Enough tickets sold");
  ...
  function refundOwnerAssets() public onlyOwner {
    ...
    ERC721EnumerableUpgradeable(prizeAddress).safeTransferFrom(address(this), owner(), prizeId);
  }
  ...
  function distributePrize() public onlyOwner {
    ...
    ERC721EnumerableUpgradeable(prizeAddress).setApprovalForAll(address(distributor), true);
    distributor.distributeToNftHolders(params.fee(), address(this), 1, tokenCounter, prizeAddress, prizeId);
    hasCalledVRF = true;
  }

  function claimETH() public onlyOwner {
    require(hasCalledVRF, "Must call VRF first");
    require(ERC721EnumerableUpgradeable(prizeAddress).ownerOf(prizeId) != address(distributor), "No VRF yet");
